...
router.post('/login', async (req, res) => {
  try {
    ...
    let allBusinesses;
    if (user.role === 'admin') {
      allBusinesses = (await query("SELECT id, name FROM businesses WHERE status = 'active' ORDER BY name")).rows;
    } else {
      allBusinesses = (await query(
        "SELECT b.id, b.name FROM businesses b JOIN user_businesses ub ON b.id = ub.business_id WHERE ub.user_id = $1 AND b.status = 'active'", [user.id]
      )).rows;
    }
...
router.get('/me', authenticate, async (req, res) => {
  try {
    ...
    const businesses = req.user.role === 'admin'
      ? (await query("SELECT id, name FROM businesses WHERE status = 'active' ORDER BY name")).rows
      : (await query("SELECT b.id, b.name FROM businesses b JOIN user_businesses ub ON b.id = ub.business_id WHERE ub.user_id = $1 AND b.status = 'active'", [req.user.id])).rows;
