...
async function initDb() {
  if (isPostgres()) {
    ...
    // origin. Set-based & self-healing: recomputes each deploy but IS DISTINCT FROM keeps
    // it to changed rows only. Second statement clears stale values for orders that no
    // longer have any delivery-action scan (e.g. rows polluted by the earlier 'A' rule).
    try {
      await query(`UPDATE orders o SET delivery_branch = sub.location
        FROM (
          SELECT DISTINCT ON (order_id) order_id, location
          FROM delivery_statuses
          WHERE status_code IN ('ATD','D','PS','UD','UDH','RS','HI','HO','RTNB') AND COALESCE(location,'') <> ''
          ORDER BY order_id, status_date DESC
        ) sub
        WHERE o.id = sub.order_id AND o.delivery_branch IS DISTINCT FROM sub.location`);
      await query(`UPDATE orders o SET delivery_branch = NULL
        WHERE o.delivery_branch IS NOT NULL AND NOT EXISTS (
          SELECT 1 FROM delivery_statuses ds
          WHERE ds.order_id = o.id
            AND ds.status_code IN ('ATD','D','PS','UD','UDH','RS','HI','HO','RTNB')
            AND COALESCE(ds.location,'') <> ''
        )`);
